...
save_path = "/home/isi/li/my_SNN/pic/acc_comparison_Fmnist_avg.png"

# 存储 x 和 y 数据
acc_values = {}  # 存储 {folder_float: [max_acc_e1, max_acc_e2, ...]}
baseline_acc = None  # 存储 baseline (0.0) 的 acc_e 最大值

# 先读取 baseline (0.0) 只读取 test1
...
y_values = [sum(values) / len(values) for values in [acc_values[x] for x in x_values]]
...
plt.ylabel("accuracy (%)")
plt.title("F-mnist accuracy (Averaged)")
plt.legend()
