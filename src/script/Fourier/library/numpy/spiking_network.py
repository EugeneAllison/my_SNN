...
class SpikingLayer(object):
    def __init__(self, dim_in, dim_out, rho, tau=20, h_th=0.4):
    ...
    def init_weight(self, **kwargs):
        ...
        #define the recurrent connection weights;
        self.W_r = np.random.uniform(
            low=-0.5, high=0.5,
            size=(self.dim_out, self.dim_out)
        )
        #compute the spectral radius: the maximum in absolute value of eigenvalue
        eigv_list = np.linalg.eig(self.W_r)[0]
        spectral_radius = np.max(np.abs(eigv_list))
        #normalize the spectral radius and specify to predefined value
        self.W_r *= self.rho / spectral_radius
    ...
    def init_state(self, inputs, keep_state=True):
        ...
        self.a_current = np.zeros(shape) #record the current output
    ...
    def forward(self, dt, inputs):
        ...
        cv = dt / self.tau
        ch = 1 - cv
        self.h = (~is_ref) * (ch * self.h + cv * self.v + np.dot(self.a_current, self.W_r) )#modify this part
...
class SpikingNetwork(object):
    def __init__(self, dims, sr, bf=0.0338):
        self.layers = []
        class_num = dims[-1]
        for dim_in, dim_out in zip(dims[:-1], dims[1:]):
            l = SpikingLayer(dim_in, dim_out,rho = sr)
            l.init_weight()
