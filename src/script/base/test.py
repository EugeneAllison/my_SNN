...
parser.add_argument("--rand_seed", type=int, default=42, help="Random seed for g(a)")
parser.add_argument("--ga_ratio", type=float, default=0.5, help="Ratio of ones in g(a)")
    ...
    for i in pbar:
        ...
        for cnt, t in enumerate(ts):
            if t < T_th:
                ...
                ga = generate_random_ga(x.shape, args.ga_ratio, args.rand_seed)
                print(f"g(a) shape: {ga.shape}")  # 打印g(a)的尺寸ga = generate_random_ga(x.shape, args.ga_ratio, args.rand_seed)
                print(f"g(a) shape: {ga.shape}")  # 打印g(a)的尺寸
                out = model.step(
