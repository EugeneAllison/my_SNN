...
parser.add_argument("--gpus", type=str, default="0")  # here! add --gpus parameter

args = parser.parse_args()

os.environ["CUDA_VISIBLE_DEVICES"] = args.gpus.replace(" ", "")
...
def run(model, X, Y, dt, T, T_th=float("inf"), callback=None):
    ...
    for i in pbar:
        ...
        model.module.reset(x, keep_state=True)  # here! use model.module.reset
        os = torch.zeros((ts.shape[0], *dh.shape), device=device)
        for cnt, t in enumerate(ts):
            if t < T_th:
                out = model.module.step(dt, x, dh)  # here! use model.module.step
            else:
                out = model.module.step(  # here! use model.module.step
...
if __name__ == "__main__":
    ...
    if torch.cuda.device_count() > 1:
        ...
        model = torch.nn.DataParallel(model)  # here! wrap model with DataParallel

    model.to(device)

    minibatch_cnt = X_train.shape[0] // args.batch_size
    model.module.record(
        size=args.n_epochs * minibatch_cnt * 2
    )  # here! use model.module.record

    fig_hist = Figure(
        figsize=(8 * len(model.module.layers), 8)
    )  # here! use model.module.layers
    fig_hist.create_grid((1, len(model.module.layers)))  # here! use model.module.layers

    def display(model):
        model.module.record()  # here! use model.module.record
        for i, l in enumerate(model.module.layers):  # here! use model.module.layers
            fig_hist[i].cla()
    ...
    for epoch in trange(args.n_epochs, leave=True):
        ...
        histogram = {
            str(i): l.histogram for i, l in enumerate(model.module.layers)
        }  # here! use model.module.layers
