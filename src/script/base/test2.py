...
parser.add_argument("--rand_seed", type=int, default=None)  # ~~~ 新增参数：随机种子 ~~~
parser.add_argument(
    "--ga_ratio", type=float, default=0.5
)  # ~~~ 新增参数：g(a) 中 1 的比例 ~~~
...
def generate_random_ga(size, ratio, seed=None):  # ~~~ 新增函数，用于生成随机 g(a) ~~~
    if seed is not None:
        torch.manual_seed(seed)
    vector = torch.zeros(size, device=device)
    num_ones = int(ratio * size)
...
def run(model, X, Y, dt, T, T_th=float("inf"), callback=None):
    ...
    for i in pbar:
        ...
        for cnt, t in enumerate(ts):
            ga = generate_random_ga(
                dh.shape[1], args.ga_ratio, args.rand_seed
            )  # ~~~ 生成随机 g(a) ~~~
            # print(f"g(a) shape: {ga.shape}")  # ~~~ 打印 g(a) 的尺寸 ~~~
            if t < T_th:
