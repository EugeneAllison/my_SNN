...
parser.add_argument("--phase", type=float, default=160)
...
parser.add_argument("--target_radius", type=float, default=0.0)
...
data_train = datasets.FashionMNIST(
    "../data/fmnist", train=True, download=True, transform=transforms.ToTensor()
)
data_eval = datasets.FashionMNIST(
    "../data/fmnist", train=False, download=True, transform=transforms.ToTensor()
)
...
if __name__ == "__main__":
    ...
    model = SpikingNetwork(
        args.net_dims, target_radius=args.target_radius, seed=args.seed
    )
